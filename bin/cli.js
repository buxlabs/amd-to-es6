...
program
  ...
  .option(
    "--quotes <type>",
    "Single, double or auto quotes in the output",
    "single"
  )
  .parse(process.argv);
...
// Validate quotes option (previously validated via regex in older commander)
if (settings.quotes && !/^(single|double|auto)$/i.test(settings.quotes)) {
  process.stderr.write(
    "Invalid value for --quotes. Allowed: single, double, auto"
  );
  process.exit(1);
}
